...
-- 테이블에 대한 코멘트
COMMENT ON TABLE account_action IS '계정 활동 정보';

-- 컬럼에 대한 코멘트
COMMENT ON COLUMN account_action.id IS 'pk';
COMMENT ON COLUMN account_action.account_id IS '계정 ID (account 테이블의 FK)';
COMMENT ON COLUMN account_action.login_count IS '로그인 횟수';
COMMENT ON COLUMN account_action.last_login_at IS '마지막 로그인 일시';
COMMENT ON COLUMN account_action.log_out_count IS '로그아웃 횟수';
COMMENT ON COLUMN account_action.last_log_out_at IS '마지막 로그아웃 일시';
COMMENT ON COLUMN account_action.created_by IS '생성자';
COMMENT ON COLUMN account_action.created_at IS '생성일시';
COMMENT ON COLUMN account_action.updated_by IS '수정자';
